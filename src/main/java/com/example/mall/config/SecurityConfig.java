...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
  ...
  // 用于配置需要拦截的 url 路径、jwt 过滤器及抛出异常后的处理器
  @Override
  protected void configure(HttpSecurity httpSecurity) throws Exception {
    httpSecurity.csrf()  // 由于使用的是 JWT, 这里不需要 csrf
        .disable()
        .sessionManagement() // 基于token，所以不需要 session
        ...
        .permitAll()
        .antMatchers(HttpMethod.OPTIONS) // 跨域请求会先进行一次 options 请求
        .permitAll()
